    ...
    for document in documents:
        ...
        for index in range(len(document)):
            ...
            if documentDetail.partner_name != '\\N':  # 往来类型
                ...
                elements = driver.find_elements_by_class_name('select2-results__option')
                if len(elements) == 0:
                    log.e('往来单位不存在', documentDetail.account_code)
                    return 1
                for element in elements:
            ...
            if documentDetail.debit_amount != '\\N':  # 钱在借方
                ...
                clearElement(driver.switch_to_active_element())
                driver.switch_to_active_element().send_keys(str(documentDetail.debit_amount))  # 如果钱在借方。 tab时直接换行
            else:
                ...
                # driver.switch_to_active_element().send_keys(Keys.TAB)
                clearElement(driver.switch_to_active_element())
                driver.switch_to_active_element().send_keys(str(documentDetail.credit_amount))
